...
var airportUtil = {
    ...
    reportTypeListOptions: function () {
        var iList = cacheMgr.reportTypeList(), optionString = '<option value="-1">--Please select an Report type--</option>';

        if (iList && iList.length > 0) {
            for (var i = 0; i < iList.length; i++)
                optionString += '<option value=' + iList[i].ReportId + '>' + iList[i].ReportName + '</option>';
        }
        return optionString;
    },
    ...
    formTypeListOptions: function(manage) {
	 ...
	 {
	optionString = '<option value=' + iList[0].formID + '>' + iList[0].formName + '</option>';
	return optionString;
	 }
	 else
	 optionString = '<option value="-1">-- Please select a form type --</option>';
        if (iList && iList.length > 0) {
            for (var i = 0; i < iList.length; i++){
				//Changes for Special Airport
				/*if(sessionStorage.isSuperAdmin !== "true"){				
					if(sessionStorage["depFormAccess"] === "true" && iList[i].formID === "D"){
						optionString += '<option value=' + iList[i].formID + '>' + iList[i].formName + '</option>';
						continue;
					}
					if(sessionStorage["arivalFormAccess"] === "true" && iList[i].formID === "A"){
						optionString += '<option value=' + iList[i].formID + '>' + iList[i].formName + '</option>';
						continue;
					}
					if(sessionStorage["depFormAccess"] === "true" && sessionStorage["arivalFormAccess"] === "true" && iList[i].formID === "-1"){
						optionString += '<option value=' + iList[i].formID + '>' + iList[i].formName + '</option>';
						continue;
					}
				}
    ...
    airlineListOptions: function(isDeparture) {
		...
		//if ( sessionStorage["IsSpecialUser"] === "true" )
		//{
		//airlineString='<option value="31" direction="EAST" flightId="3350" route="EURASIA" flightType="null" originId="45">Vietnam Airlines (VN)</option>';
        ...
        if (airlineVsProps && airlineVsProps.length > 0) {
            for (var j = 0; j < airlineVsProps.length; j++) {
        		...
        		if ( sessionStorage["IsSpecialUser"] === "true" ) {
        			if (airlineVsProps[j].name == "Vietnam Airlines (VN)") {
    ...
    initAirlineInfo: function(isDeparture, airlineID, destId, airlineInfo) {
        var airlineVsProps = cacheMgr.airlineList(isDeparture);
        var airlineId = parseInt(airlineID);
        var destID = parseInt(destId);
        var flightType = null;
        if (-1 !== airlineId) {
            for (var j = 0; j < airlineVsProps.length; j++) {
                if (airlineId === airlineVsProps[j].id) {
                    ...
                    for (var k = 0; k < airlineVsProps[j].destinations.length; k++) {
                        if (destID === airlineVsProps[j].destinations[k].destId) {
                            airlineInfo.flightType = airlineVsProps[j].destinations[k].flightType;
							airlineInfo.direction = airlineVsProps[j].destinations[k].direction;
							airlineInfo.route = airlineVsProps[j].destinations[k].route;
							airlineInfo.isInit = true;
                        }
                    }
                }
            }
...
//        edDate.data('date', dateText);
    ...
    initialiseDateSingle: function (ResponseDate) {
        var stDate = $('#' + ResponseDate),
            startInput = $('#' + ResponseDate + ' input');
...
}/*,//Changes for Special Airport/User
	isRestrictedAirport: function(){
		var name = sessionStorage.airportName, resAirport = JSON.parse(sessionStorage.restrictedAirportNames), isRestricted = false, i=0 ;
			for(i=0; i< resAirport.length; i++){
				if(resAirport[i].airPortLogin === name){
					isRestricted = true;
					break;
				}
			}
		return { 'isRestricted':isRestricted, 'airlineID':resAirport[i].airlineID };
	}*/
