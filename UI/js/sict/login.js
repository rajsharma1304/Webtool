...
function loginSuccess(res) {
    ...
    try {
        if (!res.IsValidUser) {//Not a Valid user.
			...
			sessionStorage.IsSpecialUser = res.IsSpecialUser;//Added for Special User.
			sessionStorage.AirportId = res.AirportId; //Added for Special User.
            if (res && res.LastUploadDate) {//mm/dd/yyyy/hh/mm
                var date = res.LastUploadDate.split("/"), lastuploaded, swiss;
				if(date){
					lastuploaded = moment(date[2] + "-" + date[0] + "-" + date[1] + "T" + date[3] + ":" + date[4] + ":00Z").tz('Asia/Colombo').from(moment().tz("Asia/Colombo")), //Give input as yyyymmdd
					swiss = moment(date[2] + "-" + date[0] + "-" + date[1] + "T" + date[3] + ":" + date[4] + ":00Z").tz("Europe/Amsterdam").format("MMMM Do YYYY, H:mm");//Swiss Time
					if(lastuploaded){
