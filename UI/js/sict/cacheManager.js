...
//        }
    ...
    get_interviewerList: function (loginId) {
		...
		//Adding the check for Special User
    ...
    interviewerList: function () {
		//Added the check for stringified array "[]" && the check of Special User.
        if ((sessionStorage["IsSpecialUser"] && sessionStorage.IsSpecialUser === "true") || undefined === sessionStorage.interviewerList || sessionStorage.interviewerList.length === 0 || sessionStorage.interviewerList === "[]") {
    ...
    get_reportTypeList: function () {
        //if (sessionStorage.roleId)
            reporttypeURL = "./Cache/ReportType.json";
        $.ajax({
            url: reporttypeURL,
            dataType: 'json',
            async: false,
            success: function (response) {
                if (response && response.length > 0) {
                    sessionStorage.reportTypeList = JSON.stringify(response);
                }
            }
        });
    },
    reportTypeList: function () {
        ...
        if (undefined === sessionStorage.reportTypeList || sessionStorage.reportTypeList.length === 0) {
            this.get_reportTypeList();
        }
        if (undefined !== sessionStorage.reportTypeList && sessionStorage.reportTypeList.length > 0) {
            return JSON.parse(sessionStorage.reportTypeList);
        }
    },
...
//                        });                    
    ...
    get_airlinesList: function (airportId, isDeparture) {
		var airlineURL;
		//changes done for Special User.
		if(sessionStorage["IsSpecialUser"] && sessionStorage["IsSpecialUser"] === "true" && sessionStorage["AirportId"]){
			airlineURL = "./cache/AirportAirlineList/Departure_AirportAirlineList_" + sessionStorage["AirportId"] + ".json";
			if (!isDeparture)
				airlineURL = "./cache/AirportAirlineList/Arrival_AirportAirlineList_" + sessionStorage["AirportId"] + ".json";
		}
		else{
			airlineURL = "./cache/AirportAirlineList/Departure_AirportAirlineList_" + airportId + ".json";
			if (!isDeparture)
	 ...
	 get_airlinesListforSpecialUser: function (airportId, isDeparture) {
		var airlineURL;
		//changes done for Special User.
		if(sessionStorage["IsSpecialUser"] && sessionStorage["IsSpecialUser"] === "true" && sessionStorage["AirportId"]){
			airlineURL = "./cache/AirportAirlineList/Departure_AirportAirlineList_" + sessionStorage["AirportId"] + ".json";
            ...
            success: function (response) {
                if (response && response.length > 0) {
                    ...
                    for (var i = 0; i < response.length; i++) {
                        ...
                        if (response[i].OriginId == airportId)
						{						
                            airlineVsProps.push(airlineCode);
							curAirlineID = response[i].AirlineId;
							isAdded = true;
	...
	var airlineMap,isAdded,alList,airlineVsProps = [], airportId = sessionStorage.selectedAirportId;
	if(sessionStorage["IsSpecialUser"] && sessionStorage.IsSpecialUser === "true")
	{
	  alList = this.get_airlinesListforSpecialUser(airportId, isDeparture);
	  if (undefined !== alList) {
	  airlineMap = isDeparture ? g_DepartureMap : g_ArrivalMap;
	...
	//[#72729] For the login CT, By default “Abu Dhahi” is selecting for the Origin in the Interview reports.
        if(sessionStorage.roleId !== '1'){
         ...
         airlineMap = isDeparture ? g_DepartureMap : g_ArrivalMap;
         isAdded = airlineMap.containsKey(airportId);

        if (!isAdded) {
             alList = this.get_airlinesList(airportId, isDeparture);
        ...
        if (isAdded)
            airlineVsProps = airlineMap.get(airportId);
		//Added for Speical User. But no data is available.
