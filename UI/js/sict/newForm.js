...
var newformDeparture = {
    ...
    fieldWorkRange: {
        ...
        END: EndDate			// 	Dont allow for future dates
    },
    depFormData: {},
    postSUBMISSION: [],
    ...
    newAirline: function (scBt) {
		...
		firstAirlineDestinatons = (firstAirlineDestinatons === '' || firstAirlineDestinatons === undefined) ? '<option value="-1">-- Please select an Airport --</option>' : firstAirlineDestinatons;
		airlineString = (airlineString === '' || airlineString === undefined) ? '<option value="-1">-- Please select an Airline --</option>' : airlineString;
		languageString = (languageString === '' || languageString === undefined) ? '<option value="-1">-- Please select a language --</option>' : languageString;
    ...
    addLanguage: function (e) {
        ...
        if (count < 5) {
        	...
        	$('#language' + parentId + '_' + (langcounterval + 1) ).empty().append('<option value="-1">-- Please select a language --</option>');
        	$('#language' + parentId + '_' + (langcounterval + 1) ).empty().append('<option value="-1">-- Please select a language --</option>');

        	$.each(languages, function (index, element) {
        		$('#language' + parentId + '_' + (langcounterval + 1)).append('<option value="' + element.LanguageId + '">' + element.LanguageName + '</option>');
        	});
        } else {
            me.prop("disabled", "disabled");
        }
    ...
    updateForm: function (origin, sEle, callback) {
        ...
        if (validator.validateAirportData(this.formData, false)) {
            if (validator.checkAirlines()) {
                ...
                //added by adil shaikh

               // alert(sessionStorage.mode);

                if (sessionStorage["IsSpecialUser"] === "true") {
                   // alert("Hello333");
					me.formData.Airlines[0].OriginId = $('#EditedAirportList').val();
				}
                else
                {
                    if (sessionStorage.mode == 'Arrival')
                    {
                       // me.formData.Airlines[0].OriginId = sessionStorage.selectedAirportId;
                       // alert("Arrival");
                    }
                    else
                    {
...
function initialiseStatics(iNeeded) {
	...
	if ( sessionStorage["IsSpecialUser"] === "true" ) {
		callViewFormsPageVietnam();		
	}
}

var languageString = '', airlineString = '', airlineVsProps = [], firstAirlineDestinatons = '', aircraftTypeString = '';
function initialiseAirlineStatics(isModalonPage) {
	//changes for Special Airport - Added If else Block to separate.
	var selairport = $("#AirportList").val();
	var page = window.location.pathname.substr(window.location.pathname.lastIndexOf("/") + 1);
	if(isModalonPage && sessionStorage["IsSpecialUser"] && sessionStorage["IsSpecialUser"] === "true" && (page === "departureviewform.html" || page === "arrivalviewform.html")){
		selairport = $("#EditedAirportList").val();
	}
...
function callChangeInterviewer() {
	//changes for Special Airport - Added If else Block to separate.
	var page = window.location.pathname.substr(window.location.pathname.lastIndexOf("/") + 1);
	if(sessionStorage["IsSpecialUser"] && sessionStorage["IsSpecialUser"] === "true" && (page === "departureviewform.html" || page === "arrivalviewform.html")){
		cacheMgr.selectedAirport($('#EditedAirportList').val());
		var optionString = airportUtil.interviewerListOptions();
		$('#InterviewerList').empty().append(optionString);
		initialiseAirlineStatics(true);		
	}
	else{
...
//this function is ADDED by Adil Shaikh
function callViewFormsPageVietnam() {
	var selectedAirportID = $('#EditedAirportList').val();
	cacheMgr.selectedAirport(selectedAirportID);
	//$('#InterviewerList').empty().append(airportUtil.interviewerListOptions());
