...
function createHtml() {
    ...
    htmlStr += '<ul id="sortable1" class="droptrue" style="overflow-y:auto">';
    htmlStr += '</ul>';
    htmlStr += '<ul id="sortable2" class="droptrue" style="overflow-y:auto">';
    htmlStr += '</ul>';
    htmlStr += '</div>';
    /*creating products*/
	...
	/*try{
	$( "#OthersBoxId123" ).val($(".othertextfield").val());
	$( "#OthersBoxId123" )
	  .focusout(function() {
		$(".othertextfield").val($(this).val());
	  });
	}
	catch(e){} */
}
    ...
    if(inputFormat == 'confirmit'){
        $('fieldset').children('table').children('tbody').children('tr').each(function(){
            if($(this).children(':last-child').children('input').val() == ''){
				tempStr = replaceTextWithImage($(this));
...
function onKeyUpTxt()
{
$("input.othertextfield").val($("#OthersBoxId123").val());
}

function replaceTextWithImage(partxt)
{
	var txt = partxt.children(':first-child').text()
	console.log(partxt.children().length);
	if(txt.toLowerCase() == 'otherother' || txt.toLowerCase() == 'other')
		return 'Other <input type="text" id="OthersBoxId123"  onkeyup="onKeyUpTxt()"/>';
	else if(txt.indexOf('#') >= 0)
	{
		var arrStr = txt.split('#');
		if(arrStr[1])
			return '<img src="'+arrStr[1]+'" height="56" width="140"/>';
	}		
	return txt;
}          
