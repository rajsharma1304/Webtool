...
/*				ff.setAttribute('class','smoke-base'); */
	...
	build: function (e, f) {
		...
		if (f.type !== 'signal'){
			...
			if (f.type === 'alert'){
				buttons +=
					'<button class="btn btn-pink" id="alert-ok-'+f.newid+'">'+ok+'</button>';
			} else if (f.type === 'prompt' || f.type === 'confirm'){
				if (f.params.reverseButtons) {
					buttons +=
						'<button class="btn btn-pink" id="'+f.type+'-ok-'+f.newid+'">'+ok+'</button>' +
						'<button class="btn btn-pink" id="'+f.type+'-cancel-'+f.newid+'" class="cancel">'+cancel+'</button>';				
				} else {
					buttons +=
						'<button class="btn btn-pink" id="'+f.type+'-cancel-'+f.newid+'" class="cancel">'+cancel+'</button>'+
						'<button class="btn btn-pink" id="'+f.type+'-ok-'+f.newid+'">'+ok+'</button>';
