...
///<FileName> JanitorServices.cs </FileName>
...
{
    internal class JanitorServices
    {
        ...
        public ReturnValue CreateCacheFileforTargetVsCompletesCharts(string Instance)
        {
            ...
            try
            {
                ...
                GetSemesterDateRange(ref  Date1, ref  Date2);
                //For other Instances
                int Year = 0;
                int Quarter = 0;
                ...
                if (DtLogin.Rows.Count > 1)
                {
                    for (int LoginCnt = 0; LoginCnt < DtLogin.Rows.Count; LoginCnt++)
                    {
                        ...
                        {
                            DSTargetCompletes = GetTargetandCompleteforAirportLogin(Instance,AirportLoginId, Date1, Date2);
                        }
                        else
                        {
                            DSTargetCompletes = GetTargetandCompleteforAirportLogin(Instance,AirportLoginId, Quarter, Year);
                        ...
                        if (DSTargetCompletes.Tables.Count > 2)
                        {
                            foreach (DataRow DrTarget in DSTargetCompletes.Tables[0].Rows)
                            {
                                ...
                                double BusinessTarget = 0.0;
                                if (Instance == BusinessConstants.Instance.EUR.ToString())
                                    BusinessTarget = (Target * (1 / 4.0));
                                else
                                    BusinessTarget = (Target * (1 / 3.0));
                                int MissingTarget = Target - Completes;
                                int MissingCompletes = Convert.ToInt32(Math.Round(BusinessTarget - BusinessCompletes, 0, MidpointRounding.AwayFromZero));
                                ...
                                TempMissingTargetsVsBusinessClass.MissingTarget = MissingTarget;
                                TempMissingTargetsVsBusinessClass.MissingCompletes = MissingCompletes;
                ...
                if (Instance == BusinessConstants.Instance.EUR.ToString())
                {
                    DSAll = GetAllTargetandComplete(Instance,Date1, Date2);
                }
                else
                {
                    DSAll = GetAllTargetandComplete(Instance, Quarter, Year);
                }
        ...
        /// <summary>
        /// This Function is Used to Get Semester Date Range for Dashboard Charts Data
        /// </summary>        
        private void GetSemesterDateRange(ref string Date1, ref string Date2)
        {
            const string FUNCTION_NAME = "GetQuarterAndYear";
            SICTLogger.WriteInfo(CLASS_NAME, FUNCTION_NAME, "Start");
            DateTime TempDate1 = new DateTime(), TempDate2 = new DateTime();
            try
            {
                ...
                TempDate1 = new DateTime(Year1, Month1, Day1);
                TempDate2 = new DateTime(Year2, Month2, Day2);
                Date1 = TempDate1.Date.ToString("yyyy-MM-dd");
        ...
        /// <summary>
        /// This Function is used to retrieve all the Airlines Targets and Completes for a Particular Airport Login Based on Date Range
        /// </summary>
        /// <param name="AirportId">Id of the Airport</param>
        /// <returns></returns>
        public DataSet GetTargetandCompleteforAirportLogin(string Instance, int AirportLoginId, string Date1, string Date2)
        {
            DataSet DSResult = new DataSet();
            const string FUNCTION_NAME = "GetTargetandCompleteforAirportLogin";
            try
            {
                ...
                DbCommand ObjCommand = DB.GetStoredProcCommand("GetQuarterlyTargetandCompleteforAirportLogin");
                DB.AddInParameter(ObjCommand, BusinessConstants.AIRPORTLOGINID, DbType.Int32, AirportLoginId);
                DB.AddInParameter(ObjCommand, BusinessConstants.DATE1, DbType.String, Date1);
                DB.AddInParameter(ObjCommand, BusinessConstants.DATE2, DbType.String, Date2);
                DSResult = DB.ExecuteDataSet(ObjCommand);
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public DataSet GetAllTargetandComplete(string Instance,string Date1, string Date2)
        {
            DataSet DSResult = new DataSet();
            const string FUNCTION_NAME = "GetAllTargetandComplete";
            try
